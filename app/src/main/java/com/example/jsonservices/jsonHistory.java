...
public class jsonHistory {
    public static List<HashMap<String,ArrayList>> getJsonHistory(String IP, String id) {
        HttpURLConnection conn = null;
        InputStream is = null;
        try {
            String path = "http://" + IP + "/test/getOrderInfoServlet";
            path = path + "?userId=" + id;
    ...
    private static List<HashMap<String, ArrayList>> parsejson(InputStream inputStream) throws Exception{
        ...
        for(int i=0;i<orderArray.length();i++){
            ...
            ArrayList<String> orderFriendId=new ArrayList<>();
            //userID.add(jObj.get("orderUserID").toString());
            orderID.add(jObj.get("orderID").toString());
            ...
            orderTime.add(jObj.get("orderTime").toString());
            orderFriendId.add(jObj.get("orderFriendId").toString());
